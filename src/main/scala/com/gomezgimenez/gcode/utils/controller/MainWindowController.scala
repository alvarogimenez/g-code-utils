...
import com.gomezgimenez.gcode.utils
import com.gomezgimenez.gcode.utils.{Context, Frame, Point, Util}
...
class MainWindowController {
  ...
  model.measuredFrame.set(Some(utils.Frame(
  ...
  def initialize(): Unit = {
    main_panel.setCenter(GCodePlot(model))
    ...
    menu_file_open.setOnAction((_: ActionEvent) => {
        ...
        fileChooser.getExtensionFilters.addAll(
          new ExtensionFilter("G-Code Files", "*.nc"),
          new ExtensionFilter("All Files", "*.*"))
        val selectedFile = fileChooser.showOpenDialog(Context.primaryStage)
        if (selectedFile != null) {
          val gCode = GCode.readGCode(selectedFile)
          model.originalFile.set(selectedFile.getAbsolutePath)
          model.originalGCodeData.set(gCode)
          model.originalGCodeSegments.set(GCode.gCodeToSegments(gCode))
          model.transposedGCodeSegments.set(List.empty)
          Context.primaryStage.setTitle(Util.windowTitle(Some(selectedFile.getName)))
    ...
    menu_file_save_as.setOnAction((_: ActionEvent) => {
      ...
      if(model.originalFile.get != null) {
        ...
        fileChooser.getExtensionFilters.addAll(
          new ExtensionFilter("G-Code Files", "*.nc"),
          new ExtensionFilter("All Files", "*.*"))
        val selectedFile = fileChooser.showSaveDialog(Context.primaryStage)
        if (selectedFile != null) {
